...
- **E1 wind/gust recalibration applied** (engine constants; see calibration
  memo): ladders now `wind [8,13,18,25]` / `gust [15,22,30,40]`, scaled for
  daily-max values. Ladder tag: `v83-normalT2-wgE1`. Expect the national
  paint to cool markedly — day-0 T3+ drops from ~56% to ~25% — that is the
  fix working. Genuine event days keep their tier (validated on real data:
  every T4 spike checked survived).
